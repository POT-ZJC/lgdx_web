<template>
  <div class="ElectricalTesting">
    ...
    <div class="main">
      <div class="main-box">
        <h3 class="theme-color">设备：{{ $route.query.devName || "-" }}</h3>
        <el-table
          class="main-box-table"
          :data="tableData"
          stripe
          @row-click="rowClick"
        >
    ...
    reqTbale() {
      this.$http("/api/dev/queryElectricitySafetyByPage").then((res) => {
        if (res.resultCode === "200") {
          let times = 1610726400000; //2021-01-16 00:00:00
          this.tableData = res.data.rows.map((val, index) => {
            return {
              ...val,
              update: moment(times + index * 60 * 1000 * 43).format(
                "yyyy-MM-DD hh:mm"
              ),
            };
          });
