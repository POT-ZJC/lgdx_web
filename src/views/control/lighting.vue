      ...
      <div class="main-box">
        <el-table class="main-box-table" :data="tableData" stripe>
          ...
          <el-table-column label="设备状态" header-align="center">
            <template slot-scope="scope">
              <div :class="{ warning: !+scope.row.devTage }">
                {{ +scope.row.devTage ? "正常" : "异常" }}
              </div>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async getState() {
      const res = await this.$http("/api/statistical/queryLightingStatistics");
